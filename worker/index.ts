...
async function tick() {
  const due = await prisma.bot.findMany({ where: { enabled: true, monitorKind: { not: "VALUE" }, nextCheckAt: { lte: new Date() }, OR: [{ lockedAt: null }, { lockedAt: { lt: new Date(Date.now() - 600_000) } }] }, take: 50 });
