...
export default async function EditBot({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!bot || bot.monitorKind === "VALUE") notFound();
  ...
  return <><div className="crumb"><Link href="/">Dashboard</Link><span>/</span><span>Edit Bot</span></div>
    ...
    {uptime ? <UptimeForm initial={JSON.parse(JSON.stringify(bot))}/> : <BotForm initial={JSON.parse(JSON.stringify({ ...bot, latestState: latest }))}/>}
