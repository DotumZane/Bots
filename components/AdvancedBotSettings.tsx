...
type Bot = { id: string; notifyOnHistoricalLow: boolean; notifyOnProductChange: boolean; minimumChangeMinor: number; minimumChangePercent: number; variantName: string | null; variantSelector: string | null; variantValue: string | null; channels: { notificationChannelId: string }[] };
...
export function AdvancedBotSettings({ bot, channels }: { bot: Bot; channels: Channel[] }) {
  ...
  return <form className="formCard featureCard" onSubmit={save}><div className="featureHead"><div><p className="eyebrow">Advanced monitoring</p><h2>Alerts, variants & delivery</h2><p className="help">Fine-tune meaningful changes and where alerts are sent.</p></div></div>
    ...
    <div className="formActions"><span className="saved">{message}</span><button type="button" className="button secondary" onClick={test}>Send test alert</button><button className="button primary">Save advanced settings</button></div>
  </form>;
}
